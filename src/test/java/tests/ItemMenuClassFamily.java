...
public class ItemMenuClassFamily extends Baseclass{
	...
	public void menus() {
		...
		testcase=extentreport.createTest("Item Menu/Class/Family Testcases");
		String menuitemDBvalue=jdbcTemplate.queryForObject("select Config_Value from POS_STATION_CONFIG where config_id=59 and Config_Name='Using POS Menu' and Station_Id=4", String.class);
		System.out.println("The POSmenu config value in DB= " +menuitemDBvalue);
		testcase.log(Status.INFO, "The POSmenu config value in DB= " +menuitemDBvalue);
		if(menuitemDBvalue.equalsIgnoreCase("T")) {
			...
			for(WebElement list:ItemMenuClassFamilyPage.Menulist) {
				...
				testcase.log(Status.INFO,"The POS menu lists are Present" + list.getText());
					...
					if(submenulist.getText().equalsIgnoreCase(submenu))	{
						...
						for(WebElement itemsmenu1 : ItemMenuClassFamilyPage.itemsinmenulist) {
							...
							if(itemsmenu.equalsIgnoreCase(itemsmenu1.getText())) {
								...
								testcase.log(Status.INFO, itemsmenu1.getText()+ " Items underoption is selected from menu list");
			...
			testcase=extentreport.createTest("Item class, Family Cards Testcases");
								...
								if(familyitem1.equalsIgnoreCase(family.getText())) {	
									...
									for(WebElement itemlist : ItemMenuClassFamilyPage.Items) {
										...
										if(itemlist.getText().equalsIgnoreCase(splititems)) {
											...
											testcase.log(Status.PASS, "Given Item selected from item section and its present in Billing Table");
